...
namespace LambAdmin
{
    public partial class DGAdmin
    {
        ...
        public void InitCommands()
        {
            ...
            // fl toggle
            CommandList.Add(new Command("fl", 0, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    if (sender.HasField("flyfield") && sender.GetField<int>("flyfield") == 1)
                    {
            ...
            // takecurrent <player | *filter*>
            CommandList.Add(new Command("takecurrent", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
            ...
            // takeall <player | *filter*>
            CommandList.Add(new Command("takeall", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
            ...
            // nh <player | *filter*>
            CommandList.Add(new Command("nh", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
            ...
            // hh <player | *filter*>
            CommandList.Add(new Command("hh", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
            ...
            // ih <player | *filter*>
            CommandList.Add(new Command("ih", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
                    {
                        target.SetNormalHealth(9999);
                        OnInterval(50, () =>
                        {
                            target.Health = 9999;
                            return true;
            ...
            // money <player | *filter*>
            CommandList.Add(new Command("money", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
            ...
            // ar <player | *filter*>
            CommandList.Add(new Command("ar", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
            ...
            // pk <player | *filter*>
            CommandList.Add(new Command("pk", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
                    {
                        target.SetPerk("specialty_extendedmags", true, true);
                        target.SetPerk("specialty_rof", true, true);
                        target.SetPerk("specialty_stalker", true, true);
                        target.SetPerk("specialty_lightweight", true, true);
                        target.SetPerk("specialty_longersprint", true, true);
                        target.SetPerk("specialty_fastreload", true, true);
                        target.SetPerk("specialty_quickdraw", true, true);
                        target.SetPerk("specialty_scavenger", true, true);
                        target.SetPerk("specialty_fastermelee", true, true);
                        target.SetPerk("specialty_sharp_focus", true, true);
                        target.SetPerk("specialty_armorpiercing", true, true);
                        target.SetPerk("specialty_holdbreathwhileads", true, true);
                        target.SetPerk("specialty_longerrange", true, true);
                        target.SetPerk("specialty_reducedsway", true, true);
                        target.SetPerk("specialty_moredamage", true, true);
                        target.SetField("autoRevive", true);
                        target.GiveWeapon("lightstick_mp");
                        OnInterval(100, () =>
                        {
                            target.EnableWeaponSwitch();
                            target.EnableOffhandWeapons();
            ...
            // emp <player | *filter*>
            CommandList.Add(new Command("emp", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
                    {
                        target.PlayLocalSound("US_1mc_achieve_emp");
                        target.GiveWeapon("killstreak_emp_mp", 0, false);
            ...
            // sentry <player | *filter*>
            CommandList.Add(new Command("sentry", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
            ...
            // littlebird <player | *filter*>
            CommandList.Add(new Command("littlebird", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
                    {
                        target.PlayLocalSound("US_1mc_achieve_dragonfly");
                        target.GiveWeapon("killstreak_uav_mp", 0, false);
            ...
            // helisniper <player | *filter*>
            CommandList.Add(new Command("helisniper", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
                    {
                        target.PlayLocalSound("US_1mc_achieve_heli_sniper");
                        target.GiveWeapon("killstreak_ims_mp", 0, false);
            ...
            CommandList.Add(new Command("moab", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    ...
                    foreach (Entity target in targets)
                    {
                        target.PlayLocalSound("US_1mc_achieve_moab");
                        target.GiveWeapon("killstreak_helicopter_mp", 0, false);
            ...
            // ew <player | *filter*>
            CommandList.Add(new Command("ew", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
            ...
            // pa <player | *filter*>
            //CommandList.Add(new Command("pa", 1, Command.Behaviour.Normal,
            //    (sender, arguments, optarg) =>
            //    {
            //        List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
            //        foreach (Entity target in targets)
            ...
            //            target.SetField("hasPassenger", true);
            //            //target.SetField("readyForEnter", true);
            //            target.SetField("heliTime", 120);
            //            //target.SetField("doneService", false);
            //            target.SetField("ownsHeliSniper", false);
            //            //target.SwitchToWeapon("killstreak_ims_mp");
            //            target.SetField("flying", true);
            //            target.SetField("isBeingCarried", true);
            //            ////dm salvaged code
            //            target.SetField("deathHud", true);
            //            HudElem hudElem2 = GSCFunctions.NewClientHudElem(target);
            //            hudElem2.Archived = false;
            //            hudElem2.Foreground = true;
            //            hudElem2.HideWhenInMenu = true;
            //            hudElem2.HorzAlign = HudElem.HorzAlignments.Center_Adjustable;
            //            hudElem2.VertAlign = HudElem.VertAlignments.Bottom;
            //            hudElem2.AlignX = HudElem.XAlignments.Center;
            //            hudElem2.AlignY = HudElem.YAlignments.Middle;
            //            hudElem2.X = 75f;
            //            hudElem2.Y = 0f;
            //            hudElem2.Alpha = 1f;
            ...
            // ga <player | *filter*>
            CommandList.Add(new Command("ga", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
                    {
                        ...
                        WriteChatToPlayer(sender, Command.GetString("ga", "message1").Format(new Dictionary<string, string>()
                        {
                            {"<target>", target.Name},
                        }));
                        WriteChatToPlayer(target, Command.GetString("ga", "message2").Format(new Dictionary<string, string>()
                        {
                            {"<sender>", sender.Name},
                        }));
                    }
            ...
            // gs <player | *filter*>
            CommandList.Add(new Command("gs", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
            ...
            // p1 <player | *filter*>
            CommandList.Add(new Command("p1", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
            ...
            // p2 <player | *filter*>
            CommandList.Add(new Command("p2", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
                    {
                        ...
                        target.GiveWeapon("iw5_pecheneg_mp_rof_thermal");
                        target.SetPerk("specialty_extendedmags", true, false);
                        target.SetWeaponAmmoClip(target.CurrentWeapon, 99999);
                        target.SetWeaponAmmoStock(target.CurrentWeapon, 999999);
                        target.SetField("hasAlteredROF", true);
                        target.SetClientDvar("perk_weapRateMultiplier", "0.25");
                        target.SetField("hasDeathMachine", true);
                        target.SetField("percent", 0);
            ...
            // dm <player | *filter*>
            CommandList.Add(new Command("dm", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
                    {
                        target.GiveWeapon("iw5_pecheneg_mp_rof_thermal");
                        target.SwitchToWeaponImmediate("iw5_pecheneg_mp_rof_thermal");
                        target.SetPerk("specialty_extendedmags", true, false);
                        target.SetWeaponAmmoClip(target.CurrentWeapon, 99999);
                        target.SetWeaponAmmoStock(target.CurrentWeapon, 999999);
                        target.SetField("hasAlteredROF", true);
            ...
            // tg <player | *filter*>
            CommandList.Add(new Command("tg", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
                    {
                        target.GiveWeapon("uav_strike_projectile_mp");
            ...
            // za <player | *filter*>
            CommandList.Add(new Command("za", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
            ...
            // rg <player | *filter*>
            CommandList.Add(new Command("rg", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
            ...
            // nz <player | *filter*>
            CommandList.Add(new Command("nz", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
            ...
            // mk <player | *filter*>
            CommandList.Add(new Command("mk", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
            ...
            // fmg9 <player | *filter*>
            CommandList.Add(new Command("fmg9", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
            ...
            // lg <player | *filter*>
            CommandList.Add(new Command("lg", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
            ...
            // mg <player | *filter*>
            CommandList.Add(new Command("mg", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
            ...
            // pecheneg <player | *filter*>
            CommandList.Add(new Command("pecheneg", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
            ...
            // usas <player | *filter*>
            CommandList.Add(new Command("usas", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
            ...
            // 1887 <player | *filter*>
            CommandList.Add(new Command("1887", 1, Command.Behaviour.Normal,
                (sender, arguments, optarg) =>
                {
                    List<Entity> targets = FindSinglePlayerXFilter(arguments[0], sender);
                    foreach (Entity target in targets)
            ...
            (sender, arguments, optarg) =>
            {
                ...
                WriteChatToPlayer(sender, Command.GetString("unlimitedammo", (arguments[0] == "auto") ? "message_auto" : (UTILS_ParseBool(arguments[0]) ? "message_on" : "message_off")).Format(
                            new Dictionary<string, string>()
                            {
                                {"<issuer>", sender.Name},
                                {"<issuerf>", sender.GetFormattedName(database)}
