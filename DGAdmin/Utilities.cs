        ...
        public void UTILS_UnlimitedAmmo(bool force = false)
        {
            ...
            OnInterval(50, () =>
            {
                if ((UTILS_GetDvar("unlimited_ammo") == "1") || (UTILS_GetDvar("unlimited_ammo") == "2"))
                    ...
                    foreach (Entity sender in from sender in Players select sender)
                    {
                        var Currwep = sender.CurrentWeapon;
                        var offhandAmmo = sender.GetCurrentOffhand();

                        //sender.SetWeaponAmmoClip(offhandAmmo, 99);
                        //sender.GiveMaxAmmo(offhandAmmo);

                        //sender.SetWeaponAmmoClip(Currwep, 99);

                        sender.SetWeaponAmmoClip(Currwep, 999);
                        sender.SetWeaponAmmoStock(Currwep, 9999);
                        sender.SetWeaponAmmoClip(offhandAmmo, 999);
                        sender.SetWeaponAmmoStock(offhandAmmo, 9999);
